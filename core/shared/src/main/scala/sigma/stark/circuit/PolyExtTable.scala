...
/** One step of the RISC0 recursion-circuit constraint program.
  ...
  * exactly; reduction mod P happens at interpretation time, mirroring
  * upstream's `Elem::from_u64`.
  */
...
/** The RISC0 recursion circuit's constraint system as data — the parsed
  ...
  *
  * The `ops` array is exposed for the interpreter's indexed while-loop and
  * must not be mutated.
  */
final class PolyExtTable private (
  ...
  def canonicalBytes: Array[Byte] = {
    val sb = new java.lang.StringBuilder(ops.length * 16)
    var i = 0
    while (i < ops.length) {
      val (name, operands) = PolyExtTable.serializeOp(ops(i))
  ...
  private def parseChecked(lines: Iterator[String]): Either[String, PolyExtTable] = {
    ...
    while (lines.hasNext) {
      ...
      if (line.nonEmpty && !line.startsWith("#")) {
        // -1 keeps trailing empty fields: `True` rows end in an empty operand field.
        val f = line.split("\t", -1)
        f(0) match {
          case "meta" =>
          ...
          case "op" =>
            ...
            val operands: Array[Long] =
              if (f(3).isEmpty) Array.empty[Long] else f(3).split(",", -1).map(_.toLong)
