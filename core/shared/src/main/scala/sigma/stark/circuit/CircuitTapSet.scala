...
object CircuitTapSet {
  ...
  private def parseChecked(lines: Iterator[String]): Either[String, CircuitTapSet] = {
    ...
    if (comboBegin.length != combosCount + 1 || comboBegin(0) != 0)
      return Left(s"circuit_taps: combo_begin must have ${combosCount + 1} entries starting at 0")
    ...
    if (comboTaps.length != totComboBacks)
